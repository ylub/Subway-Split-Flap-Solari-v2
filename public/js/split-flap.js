...
(sf.items = {
  ...
  (sf.display = {
     ...
     */
    loadRow: (data, row) => {
      // Play split-flap sound for this row if sound is enabled
      if (window.splitFlapSound && window.splitFlapSound.enabled) {
        window.splitFlapSound.play();
      }
      ...
      groups.each(group => {
        ...
        const c = $(groups[group])
          .attr('class')
          .split(/\s+/)[1];
        const d = (data[c] !== undefined && data[c] !== null) ? data[c] : '';
